        ...
        (async () => {
            try {
                if (mediaType === 'audio' && uploadData.ptt) {
                    const buffer = await fs_1.promises.readFile(bodyPath);
                    const opusBuffer = await (0, messages_media_1.convertAudioToOpus)(buffer);
                    await fs_1.promises.writeFile(bodyPath, opusBuffer);
                }
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message && !!message.buttons) {
        ...
        if ('contextInfo' in message && !!message.contextInfo) {
            buttonsMessage.contextInfo = { ...(buttonsMessage.contextInfo || {}), ...message.contextInfo };
        }
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        ...
        if ('contextInfo' in message && !!message.contextInfo) {
            interactiveMessage.contextInfo = { ...(interactiveMessage.contextInfo || {}), ...message.contextInfo };
        }
        if ('mentions' in message && !!message.mentions) {
...
const getInteractiveMessageContent = (content) => {
    let type;
    const attrs = {};
    let contentNode;
    if (content.buttons) {
        type = 'buttons';
        contentNode = content.buttons.map(b => ({
            tag: 'button',
            attrs: {
                id: b.buttonId,
            },
        ...
        type = 'template';
        contentNode = content.templateButtons.map(b => {
            const btn = {};
            if (b.index) {
                btn.index = b.index;
            }
        ...
        contentNode = [
            {
                tag: 'list',
                attrs: {
                    type: content.listMessage.listType,
                },
                content: [
                    {
                        tag: 'sections',
                        attrs: {},
                        content: content.listMessage.sections.map(s => ({
                            tag: 'section',
                            attrs: {
                                title: s.title,
                            },
                            content: s.rows.map(r => ({
                                tag: 'row',
                                attrs: {
                                    rowId: r.rowId,
                                    title: r.title,
...
exports.getInteractiveMessageContent = getInteractiveMessageContent;
/**
