...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, groupMetadata: getGroupMetadataFromSocket, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, createParticipantNodes, getUSyncDevices, sendPeerDataOperationMessage, } = sock;
    const lidCache = new node_cache_1.default({ stdTTL: 3600, useClones: false });
    const groupMetadataCache = new node_cache_1.default({ stdTTL: 3600, useClones: false });
    ...
    const groupMetadata = async (jid) => {
        ...
        if (!metadata) {
            metadata = await getGroupMetadataFromSocket(jid);
            if (metadata) {
    ...
    const handleMessage = async (node) => {
        ...
        // retry msmsg messages, instead of ignoring them
        if (encNode && encNode.attrs.type === 'msmsg') {
            logger.debug({ key: node.attrs.key }, 'msmsg, sending retry request');
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (msg.key.participant && (0, WABinary_1.isLidUser)(msg.key.participant)) {
                            msg.key.participant = lidCache.get(msg.key.participant) || node.attrs.participant_pn || msg.key.participant;
                        }
                        if (msg.key.remoteJid && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            msg.key.remoteJid = lidCache.get(msg.key.remoteJid) || node.attrs.sender_pn || node.attrs.peer_recipient_pn || msg.key.remoteJid;
                        }
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid)) {
                            const contextInfo = msg.message?.extendedTextMessage?.contextInfo;
                            if (contextInfo?.participant && (0, WABinary_1.isLidUser)(contextInfo.participant)) {
                                const jid = lidCache.get(contextInfo.participant);
                                if (jid) {
                                    contextInfo.participant = jid;
                                }
                                else {
                                    const metadata = await groupMetadata(msg.key.remoteJid);
                                    const participant = metadata.participants.find(p => p.id === contextInfo.participant);
                                    contextInfo.participant = (participant === null || participant === void 0 ? void 0 : participant.jid) || contextInfo.participant;
                                }
                            }
                        }
                        ...
                        await (0, Utils_1.delay)(Math.floor(Math.random() * 450) + 50);
